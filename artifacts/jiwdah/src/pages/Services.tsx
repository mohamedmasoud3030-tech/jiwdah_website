...
const COMPLETE = SERVICES.slice(2);
  ...
  return (
    <div className="min-h-screen bg-surface">
      ...
      <main className="pt-20">
        ...
        {/* Signature Experiences — 2 premium full-bleed editorial panels */}
        <section className="py-20 px-6 lg:px-8">
          <div className="max-w-7xl mx-auto">
              ...
              {SIGNATURE.map((service, index) => {
                ...
                return (
                  <motion.div
                    ...
                    className="group border border-gold/15 rounded overflow-hidden cursor-pointer hover:border-gold/30 transition-all duration-500"
                    style={{ background: "linear-gradient(135deg, #1a1a16 0%, #161610 100%)" }}
                    onClick={() => setSelectedService(service)}
                  >
                    <div className={`grid grid-cols-1 md:grid-cols-2 ${index % 2 === 1 ? "md:[direction:ltr]" : ""}`}>
                      <div className="relative aspect-[4/3] md:aspect-auto overflow-hidden min-h-64">
                        ...
                        />
                        <div className={`absolute inset-0 ${index % 2 === 1 ? "bg-gradient-to-r" : "bg-gradient-to-l"} from-[#1a1a16] to-transparent`} />
                      </div>
                      <div className="p-10 lg:p-14 flex flex-col justify-center" dir="rtl">
                        <div className="flex items-center gap-3 mb-4">
                          <div className="w-9 h-9 bg-gold/10 border border-gold/20 rounded flex items-center justify-center">
                            <IconComponent className="w-4 h-4 text-gold" />
                          </div>
                          <span className="text-gold/55 text-[10px] tracking-widest uppercase border border-gold/15 px-2.5 py-1 rounded">
                            {`0${index + 1}`} — حصري
                          </span>
                        </div>
                        <h2
                        ...
                        <div className="flex items-center gap-6">
                          <span className="text-gold/60 text-sm">
                            يبدأ من <strong className="text-gold text-lg">{service.price} ر.ع</strong>
                          </span>
                          <Link
                            to="/contact"
        ...
        {/* Complete Services — secondary editorial list */}
        {COMPLETE.length > 0 && (
          <section className="py-20 px-6 lg:px-8">
            <div className="max-w-7xl mx-auto">
              <motion.div variants={fadeSlideUp} initial="hidden" whileInView="visible" viewport={{ once: true }} className="flex items-center gap-3 mb-10">
                      ...
                      >
                        ...
                        <div className="flex-1 flex flex-col justify-center">
                          <div className="flex items-center gap-3 mb-3">
                            ...
                            <span className="text-gold/45 text-xs tracking-widest uppercase">{`0${SIGNATURE.length + index + 1}`}</span>
                          </div>
                          <h2
                          ...
                          <div className="flex items-center gap-6">
                            <span className="text-gold/55 text-sm">
                              يبدأ من <strong className="text-gold">{service.price} ر.ع</strong>
                            </span>
